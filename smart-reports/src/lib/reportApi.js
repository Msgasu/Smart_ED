/**
 * Smart Reports Frontend - Report API Client
 * Interfaces with the backend report management API
 */

import { supabase } from './supabase'

/**
 * Report Status Constants (matching backend)
 */
...
/**
 * Get reports by status with filtering
 * @param {string} status - Report status to filter by
 * @param {Object} filters - Additional filters
 * @returns {Promise<Object>} Reports data or error
 */
export const getReportsByStatus = async (status = REPORT_STATUS.COMPLETED, filters = {}) => {
  try {
    ...
    // For students, only show their own reports
    ...
    // Add student profile information if needed
    if (reports && reports.length > 0 && userProfile?.role !== 'student') {
 ...
 */
export const getReportById = async (reportId, includeIncomplete = false) => {
  try {
    const user = await getCurrentUser()
    if (!user) {
      return { data: null, error: 'User not authenticated' }
    ...
    if (userProfile?.role === 'student' && report.student_id !== user.id) {
      return { data: null, error: 'Unauthorized to view this report' }
    }

    // For non-admin users, only show completed reports unless specifically requested
    if (userProfile?.role !== 'admin' && !includeIncomplete && report.status !== REPORT_STATUS.COMPLETED) {
      return { 
        data: { 
          ...report, 
          incomplete: true,
...
export const completeReport = async (reportId) => {
  try {
    ...
    if (userProfile?.role !== 'admin') {
      return { data: null, error: 'Only administrators can complete reports' }
    }
...
/**
 * Revert a report to draft status (admin only)
 * @param {string} reportId - Report ID
 * @param {string} reason - Reason for reverting
 * @returns {Promise<Object>} Updated report data or error
 */
export const revertReportToDraft = async (reportId, reason) => {
  try {
    const user = await getCurrentUser()
    if (!user) {
      return { data: null, error: 'User not authenticated' }
    ...
    if (userProfile?.role !== 'admin') {
      return { data: null, error: 'Only administrators can revert reports' }
    }
 ...
 */
export const canEditReport = async (reportId) => {
  try {
    const user = await getCurrentUser()
    if (!user) {
    ...
    if (!['admin', 'faculty'].includes(userProfile?.role)) {
...
export const saveReport = async (reportData) => {
  try {
    ...
    if (reportData.id) {
      const { canEdit, reason } = await canEditReport(reportData.id)
      if (!canEdit) {
